      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-100">骑行天气与路线气象顾问</h1>
            <p className="text-slate-400 text-sm mt-1">
              结合实时气象与顺逆风判定，精准计算沿途各路段到达时刻的气温、降雨概率、风阻及出行穿衣建议。
            </p>
          </div>
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-slate-200 text-xs font-semibold border border-slate-700/80 cursor-pointer transition shadow-sm">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-5">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-cyan-400 font-mono font-medium truncate max-w-[180px]" title={customRouteName}>
                {customRouteName}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-slate-300 block mb-1">计划出发时间</label>
                <input
                  type="time"
                  value={departureTime}
                  onChange={(e) => setDepartureTime(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-sm text-cyan-400 font-mono"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-slate-300 block mb-1">预计均速 (km/h)</label>
                <input
                  type="number"
                  value={avgSpeedKmh}
                  onChange={(e) => setAvgSpeedKmh(parseFloat(e.target.value) || 25)}
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-400 font-medium">行者精选·浙江经典实测路线:</span>
                <label className="text-[11px] text-cyan-400 hover:text-cyan-300 cursor-pointer font-medium flex items-center gap-1">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                {ZHEJIANG_XINGZHE_ROUTES.map((r) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-500 block font-mono">{r.distanceKm}km | {r.city}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center text-xs text-slate-400">
                ...
                <span className="font-mono text-cyan-400">{routePoints.length} 个航点</span>
              </div>
              <div ref={mapContainerRef} className="w-full h-72 rounded-xl border border-slate-800"></div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          {weatherSegments.length === 0 ? (
            <div className="glass-panel p-12 rounded-2xl border border-slate-800 text-center flex flex-col items-center justify-center space-y-3">
              ...
              <h3 className="text-base font-semibold text-slate-200">暂无路段气象数据</h3>
            ...
            <div className="space-y-4">
              ...
              <div className="glass-panel p-4 rounded-xl border border-slate-800 bg-cyan-950/20 flex items-start gap-3">
                <ShieldCheck className="w-5 h-5 text-cyan-400 shrink-0 mt-0.5" />
                <div className="space-y-1 text-xs">
                  <span className="font-bold text-cyan-300 block">智能装备与补水补给建议</span>
                  <p className="text-slate-300">
                    全程最高气温约 <strong>{maxTemp}°C</strong>，紫外线峰值 <strong>UV {maxUv}</strong>。
              ...
              <div className="space-y-3">
                {weatherSegments.map((seg) => (
                  <div key={seg.pointIndex} className="glass-panel p-4 rounded-xl border border-slate-800 bg-slate-900/60 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-slate-200">
                          {seg.distanceKm === 0 ? '出发起点' : `路程 ${seg.distanceKm} km 处`}
                        </span>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-slate-400 block text-[10px]">气温 / 体感</span>
                          <span className="text-slate-200 font-mono font-semibold">{seg.temp}°C / {seg.feelsLike}°C</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-slate-400 block text-[10px]">风速风向</span>
                          <span className="text-slate-200 font-mono font-semibold">{seg.windSpeedKmh} km/h ({seg.windDirectionDeg}°)</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-slate-400 block text-[10px]">湿度 / 降水率</span>
                          <span className="text-slate-200 font-mono font-semibold">{seg.humidity}% / {seg.precipProb}%</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <span className="text-slate-400 block text-[10px]">紫外线指数</span>
                          <span className="text-slate-200 font-mono font-semibold">UV {seg.uvIndex} ({seg.uvIndex >= 6 ? '强' : '中等'})</span>
                        </div>
                      </div>
                    </div>
                  </div>
