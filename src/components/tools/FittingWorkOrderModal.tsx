...
import { X, Printer, Check, Calendar, User, Wrench, Bike, FileText } from 'lucide-react';
import { RouleurLogo } from '../common/RouleurLogo';

interface FittingWorkOrderModalProps {
  isOpen: boolean;
  onClose: () => void;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-md flex items-center justify-center p-3 sm:p-5 no-print animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#1C1C1E] border border-black/[0.08] dark:border-white/10 rounded-2xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-ios-popover overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-black/[0.06] dark:border-white/10 bg-slate-50 dark:bg-[#2C2C2E]/50 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="apple-touch h-9 px-4 rounded-xl bg-ios-blue hover:bg-ios-blue/90 text-white font-semibold text-xs transition shadow-ios-sm flex items-center gap-1.5"
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-200/60 dark:bg-black/60 flex justify-center">
          <div
            id="fitting-printable-sheet"
            className="w-full max-w-[210mm] bg-white text-slate-900 shadow-ios-card rounded-sm p-6 sm:p-8 space-y-3.5 text-xs font-sans select-none"
            style={{ minHeight: '275mm' }}
          >
            {/* Sheet Header: Logo + Title + Order Meta */}
            <div className="flex items-start justify-between border-b-2 border-slate-900 pb-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-slate-900 text-white flex items-center justify-center shrink-0">
                  <RouleurLogo className="w-6 h-6 text-white" />
                </div>
                <div>
                  <div className="flex items-center gap-1.5">
                    <span className="font-bold text-base tracking-wider font-mono text-slate-900">ROULEUR PRO</span>
                    <span className="text-[10px] px-1.5 py-0.2 rounded bg-slate-900 text-white font-mono font-bold">WORK ORDER</span>
                  </div>
            ...
            {/* Rider & Bike Info Bar */}
            <div className="grid grid-cols-3 gap-3 p-2 bg-slate-100 rounded border border-slate-200 text-xs">
              <div>
                <span className="text-slate-500 block text-[10px]">车手姓名 / RIDER:</span>
                <span className="font-bold text-slate-900 text-xs">{riderName}</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px]">装车技师 / FITTER:</span>
                <span className="font-bold text-slate-900 text-xs">{fitterName}</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px]">车架型号 / BIKE MODEL:</span>
                <span className="font-bold text-slate-900 text-xs">{bikeModel}</span>
              </div>
            </div>

            {/* Section 1: Biometric Measurements */}
            ...
            {/* Section 2: Recommended Frame & Cockpit Geometry */}
            <div className="space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900 border-b border-slate-300 pb-0.5">
            ...
            {/* Section 3: Clean Vector Bike Geometry Schematic */}
            <div className="space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900 border-b border-slate-300 pb-0.5">
                <Bike className="w-3.5 h-3.5 text-slate-700" />
                <span>三、装车关键基准线拟合图解 (Fitting Geometry Schematic)</span>
              </div>
              <div className="border border-slate-200 rounded p-1.5 bg-slate-50/50 flex justify-center">
                <svg viewBox="0 0 500 185" className="w-full max-w-[430px] h-auto text-slate-800">
                  {/* Ground */}
                  <line x1="20" y1="165" x2="480" y2="165" stroke="#CBD5E1" strokeWidth="1" strokeDasharray="3 3" />

                  {/* Rear Wheel (80, 130) */}
                  <circle cx="80" cy="130" r="35" fill="none" stroke="#64748B" strokeWidth="3" />
                  <circle cx="80" cy="130" r="4" fill="#1E293B" />

                  {/* Front Wheel (420, 130) */}
                  <circle cx="420" cy="130" r="35" fill="none" stroke="#64748B" strokeWidth="3" />
                  <circle cx="420" cy="130" r="4" fill="#1E293B" />

                  {/* BB: (210, 130) */}
                  <circle cx="210" cy="130" r="6" fill="#0F172A" />

                  {/* Frame Lines */}
                  <line x1="80" y1="130" x2="210" y2="130" stroke="#0F172A" strokeWidth="3" />
                  <line x1="210" y1="130" x2="185" y2="68" stroke="#0F172A" strokeWidth="3.5" />
                  <line x1="185" y1="68" x2="175" y2="38" stroke="#475569" strokeWidth="2.5" />
                  <path d="M 155 35 Q 175 35 195 37" stroke="#0F172A" strokeWidth="4" fill="none" strokeLinecap="round" />

                  <line x1="345" y1="68" x2="355" y2="52" stroke="#0F172A" strokeWidth="4" />
                  <line x1="210" y1="130" x2="345" y2="68" stroke="#0F172A" strokeWidth="3.5" />
                  <line x1="185" y1="68" x2="345" y2="68" stroke="#0F172A" strokeWidth="3" />
                  <line x1="80" y1="130" x2="185" y2="68" stroke="#0F172A" strokeWidth="2.5" />
                  <line x1="345" y1="68" x2="420" y2="130" stroke="#0F172A" strokeWidth="3" />

                  <line x1="355" y1="52" x2="385" y2="52" stroke="#475569" strokeWidth="3" strokeLinecap="round" />
                  <circle cx="385" cy="54" r="3.5" fill="#0F172A" />

                  {/* Dimension Line: Saddle Height */}
                  <line x1="215" y1="130" x2="180" y2="38" stroke="#2563EB" strokeWidth="1.5" strokeDasharray="2 2" />
                  <text x="175" y="88" fill="#1D4ED8" fontSize="8.5" fontWeight="bold" fontFamily="monospace">坐高 {data.saddleHeight}cm</text>

                  {/* Dimension Line: ETT */}
                  <line x1="185" y1="62" x2="345" y2="62" stroke="#059669" strokeWidth="1.2" />
                  <text x="245" y="58" fill="#059669" fontSize="8.5" fontWeight="bold" fontFamily="monospace">ETT {data.effectiveTopTube}cm</text>

                  {/* Dimension Line: Drop */}
                  <line x1="175" y1="35" x2="400" y2="35" stroke="#7C3AED" strokeWidth="1" strokeDasharray="2 2" />
                  <line x1="385" y1="54" x2="400" y2="54" stroke="#7C3AED" strokeWidth="1" strokeDasharray="2 2" />
                  <line x1="396" y1="35" x2="396" y2="54" stroke="#7C3AED" strokeWidth="1.5" />
                  <text x="403" y="47" fill="#7C3AED" fontSize="8" fontWeight="bold" fontFamily="monospace">落差 {data.saddleDrop}cm</text>
                </svg>
              </div>
            </div>

            {/* Section 4: Cleats & Professional Installation Notes */}
            <div className="space-y-1">
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-900 border-b border-slate-300 pb-0.5">
            ...
            {/* Section 5: Signature & Guarantee Confirmation */}
            <div className="pt-2 border-t-2 border-slate-900 grid grid-cols-3 gap-4 text-xs">
              <div>
                <span className="text-slate-500 block text-[10px] mb-3">主调技师签名 / FITTER SIGN:</span>
                <div className="border-b border-slate-400 font-mono text-xs pb-0.5 font-bold">{fitterName}</div>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] mb-3">车手客户确认 / RIDER SIGN:</span>
                <div className="border-b border-slate-400 font-mono text-xs pb-0.5 font-bold">{riderName}</div>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] mb-3">复检保障 / WARRANTY:</span>
