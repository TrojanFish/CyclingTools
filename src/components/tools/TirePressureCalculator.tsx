...
import { Gauge, Info, AlertTriangle, Layers, Copy, Check } from 'lucide-react';
...
export const TirePressureCalculator: React.FC = () => {
  ...
  const [riderWeight, setRiderWeight] = useState<number>(68);
  const [bikeGearWeight, setBikeGearWeight] = useState<number>(8.5);
      ...
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-100">公路/全地形智能胎压计算器</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 space-y-6">
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-5">
            ...
            {/* Bike Type Selector */}
            <div>
              <label className="text-xs font-medium text-slate-300 block mb-2">车辆类型 (Bike Type)</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="text-xs font-medium text-slate-300">车手净体重 (kg)</label>
                  <span className="text-cyan-400 font-mono font-semibold text-xs">{riderWeight} kg</span>
                </div>
                <input
                  type="number"
                  step="0.5"
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="text-xs font-medium text-slate-300">车重 + 装备水壶 (kg)</label>
                  <span className="text-cyan-400 font-mono font-semibold text-xs">{bikeGearWeight} kg</span>
                </div>
                <input
                  type="number"
                  step="0.5"
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-cyan-400 font-mono font-semibold text-xs">前 {weightDistFront}% / 后 {weightDistRear}%</span>
              </div>
              <input
                type="range"
                min="38"
            ...
            {/* Tire Setup */}
            <div>
              <label className="text-xs font-medium text-slate-300 block mb-2">轮胎系统 (Tire Setup)</label>
              <div className="grid grid-cols-3 gap-2">
                {[
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="text-xs font-medium text-slate-300 block mb-1">标称胎宽 (mm)</label>
                <select
                  value={nominalWidth}
                  onChange={(e) => {
              ...
              <div>
                <label className="text-xs font-medium text-slate-300 block mb-1">实测胎宽 (mm)</label>
                <input
                  type="number"
                  step="0.5"
                  value={actualWidth}
              ...
              <div>
                <label className="text-xs font-medium text-slate-300 block mb-1">车圈内宽 (mm)</label>
                <input
                  type="number"
                  step="0.5"
                  value={rimInnerWidth}
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-slate-800">
              <div>
                <span className="text-xs font-semibold text-slate-200 block">无钩车圈 (Hookless Rim)</span>
                <span className="text-[10px] text-slate-400">ETRTO 强制上限 72.5 PSI (5.0 Bar)</span>
              </div>
              <input
                type="checkbox"
                checked={isHookless}
            ...
            {/* Surface Type */}
            <div>
              <label className="text-xs font-medium text-slate-300 block mb-2">主要路面条件 (Surface Conditions)</label>
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-panel p-5 rounded-2xl border border-slate-800 bg-slate-900/60">
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-semibold text-cyan-400 uppercase">前轮建议区间</span>
              </div>
              <div className="text-2xl font-bold font-mono text-slate-100 my-1">
                {result.front.min} - {result.front.max} <span className="text-xs text-slate-400 font-sans font-normal">{pressureUnit.toUpperCase()}</span>
              </div>
            ...
            <div className="glass-panel p-5 rounded-2xl border border-slate-800 bg-slate-900/60">
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-semibold text-cyan-400 uppercase">后轮建议区间</span>
              </div>
              <div className="text-2xl font-bold font-mono text-slate-100 my-1">
                {result.rear.min} - {result.rear.max} <span className="text-xs text-slate-400 font-sans font-normal">{pressureUnit.toUpperCase()}</span>
              </div>
